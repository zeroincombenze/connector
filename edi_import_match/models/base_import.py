...
class BaseImportMatch(models.Model):
    ...
    field_ids = fields.One2many(
        comodel_name="edi.import.match.field",
    ...
    def _usable_rules(self, model_name, fields):
        ...
        for record in available:
            if all(f.name in fields or f.default_value for f in record.field_ids):
                result |= record
        return result


class BaseImportMatchField(models.Model):
    _name = "edi.import.match.field"
    ...
    default_value = fields.Char(help="Default value if field not in imported file.")
