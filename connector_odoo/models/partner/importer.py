...
class PartnerBatchImporter(Component):
    ...
    def run(self, filters=None):
        ...
        _logger.info("search for odoo partner %s returned %s", filters, external_ids)
...
class PartnerImportMapper(Component):
    ...
    @mapping
    def fiscalcode(self, record):
        if record.parent_id:
            return {"fiscalcode": False}
        return {"fiscalcode": record.fiscalcode}
    ...
    def category(self, record):
        ...
        if parent:
            ...
            if not bind_parent:
                raise MappingError(
                    "The parent partner with odoo id %s is not imported." % parent.name
                )
