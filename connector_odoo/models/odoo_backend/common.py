...
class OdooBackend(models.Model):
    ...
    default_import_tax = fields.Boolean(string="Import Taxes")
    import_taxes_from_date = fields.Datetime("Import tax from date")

    default_import_sale_order = fields.Boolean(string="Import Sale Orders")
    import_sale_orders_from_date = fields.Datetime("Import sale order from date")
    ...
    @api.multi
    def import_product_uom_uom(self):
        if not self.default_import_product:
            return False
        self._import_from_date("odoo.uom.uom")
        return True
    ...
    def import_product_attributes(self):
        if not self.default_import_product:
            return False
        self._import_from_date("odoo.product.attribute")
        return True

    @api.multi
    def import_product_attribute_values(self):
        if not self.default_import_product:
            return False
        self._import_from_date("odoo.product.attribute.value")
        return True

    @api.multi
    def import_account_tax(self):
        if not self.default_import_tax:
            return False
        self._import_from_date("odoo.account.tax", "import_taxes_from_date")
        return True
    ...
    def import_sale_order(self):
        if not self.default_import_sale_order:
            return False
        self._import_from_date("odoo.sale.order", "import_sale_orders_from_date")
        return True
    ...
    @api.multi
    def _import_from_date(self, model, from_date_field=None):
                ...
                if from_date:
                    filters.append(
                        (
                            "write_date",
                            ">",
                            import_start_time.strftime(DEFAULT_SERVER_DATETIME_FORMAT),
                        )
                    )
            # else:
            #     from_date = None
