...
class IrModelSynchro(models.Model):
    ...
    def _build_unique_index(self, model, prefix):
        ...
        self._cr.execute(  # pylint: disable=E8103
            "UPDATE %s set %s_id=NULL where %s_id=0" %
            (table, prefix, prefix)
        )
    ...
    @api.model
    def create_n_commit(self, vals, context=None):
        try:
            if context:
                rec = self.with_context(context).create(vals)
            else:
    ...
    def get_channel_model(self, channel_id, model):
        channel_model = self.env['synchro.channel.model'].search(
            [('synchro_channel_id', '=', channel_id),
             ('name', '=', model)])
        return channel_model[0] if channel_model else self.env[
        ...
        if (len(req_domain) == 1 and
            ...
            if rec:
                ...
                return cls.with_context(
                    {'lang': self.env.user.lang}).browse(rec.res_id), maybe_dif
    ...
    def get_foreign_text(self, channel_id, actual_model, value, is_foreign,
        ...
        if rec:
            new_value = rec[0].id
        if not new_value and cache.is_manageable(actual_model):
    ...
    def get_foreign_ref(self, channel_id, actual_model, value_id, is_foreign,
        ...
        if not new_value and cache.is_manageable(xmodel):
            new_value = self.create_new_ref(
    ...
    def get_foreign_value(self, channel_id, xmodel, value, name, is_foreign,
        ...
        # TODO: channel_id
        cache.open(channel=self.env['synchro.channel'].browse(channel_id),
            model=relation)
...
#            return self.assign_channel(vals)
    ...
    def synchro(self, cls, vals, disable_post=None, no_deep_fields=None,
        ...
        if loc_id < 1:
            ...
            if vals:
                # for item in (
                #         'vg7_id', 'vg72_id', 'oe7_id', 'oe8_id', 'oe10_id'):
                #     if (cache.get_struct_model_attr(actual_model, item) and
                #             item not in vals):
                #         vals[item] = getattr(rec, item) if rec else False
                        # if (not vals[item] and
                        #         isinstance(vals[item], (int, long))):
                        #     vals[item] = False
    ...
    def synchro_queue(self, channel_id):
        ...
        while queue:
            ...
            rec = self.get_actual_model(xmodel).with_context(
                {'lang': self.env.user.lang}).browse(loc_id)
            loc_ext_id_name = self.get_loc_ext_id_name(channel_id, xmodel)
