...
class SynchroApi(models.Model):
    ...
    get_data_endpoint(backend, data_path=None): return data endpoint from login endpoint
    ...
    def odoo_xmlrpc_https_get_data_endpoint(self, backend, data_path=None):
        if backend.counterpart_url:
            return backend.counterpart_url.replace("/common", "/object")
        return False  # pragma: no cover

    def get_data_endpoint(self, backend, data_path=None):
        fct = self.get_overridden_fct(backend, "get_data_endpoint")
        if fct:
            return _u(getattr(self, fct)(backend, data_path=data_path))
    ...
    def odoo_xmlrpc_http_get_data_endpoint(self, backend, data_path=None):
        return self.odoo_xmlrpc_https_get_data_endpoint(backend, data_path=data_path)
    ...
    def get_default(self, backend):
        # Return default protocol, port, database, login password, login_path, data_path
    ...
    def get_login_endpoint_odoo_https(self, backend, with_port=None, rebuild=False):
        if not rebuild and backend.counterpart_url and not backend.hostname:
            ...
            if with_port and backend.port:
                endpoint += ":%d" % backend.port
            if backend.exchange_path:
                endpoint += backend.exchange_path
        else:
            endpoint = ""
        return endpoint
