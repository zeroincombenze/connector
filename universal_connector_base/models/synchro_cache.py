...
class IrModelSynchroCache(models.Model):
    ...
    @api.model_cr_context
    def que_push(self, backend, action, model, spec, values, ttl, ctx, prio=2):
        que_name = "IN_QUEUE%d" % prio
        ...
        if action in ("synchro", "trigger", "pull"):
            ...
            for qaction, qmodel, qspec, qvalues, que_ttl, que_ctx in in_queue:
                if (action, model, spec, values) == (qaction, qmodel, qspec, qvalues):
                    found_in_que = True
                    break
            if not found_in_que:
                in_queue.append((action, model, spec, values, ttl, ctx))
    ...
    def get_channel_list(self):
        return self.env["synchro.backend"].search([], order="sequence")
