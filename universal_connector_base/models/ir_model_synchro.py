...
class IrModelSynchro(models.Model):
    ...
    def atomic_search(self, cls, domain):
        if not domain:
            return []
        try:
            if hasattr(cls, "sequence"):
                rec = cls.search(domain, order="sequence,id", limit=16)
            else:
                rec = cls.search(domain, limit=16)
    ...
    def exec_search(self, cls, domain):
        rec = self.atomic_search(cls, domain)
    ...
    def bind_record(self, synchro_model, actual_cls, vals, ctx=None):
        ...
        if synchro_model.name == "res.company":
            ...
            if len(rec) == 1:
                candidate = rec[0]
        prio = 8
        for keys in eval(synchro_model.search_keys):
            domain = []
            ...
            if domain:
                if synchro_model.search_with_company and vals.get("company_id"):
                    domain.append("|")
                ...
                rec = self.exec_search(actual_cls, domain)
                if len(rec) == 1:
                    break
                elif 1 < len(rec) < prio:
                    candidate = rec[0]
    ...
    def drop_protected_equal_fields(self, backend, vals, rec):
        ...
        for loc_name, value in vals.copy().items():
            ...
            protect_update = (
                ...
                if synchro_field
                else SynchroField.get_default_protection(loc_name, vmodel=rec._name)
            )
            if (
                protect_update == "3"
                or (
    ...
    def synchro(
        ...
        vals, min_vals = synchro_model.map_to_internal(
            vals,
            ttl,
        ...
        if not rec:
            rec = -1
            if synchro_model.auth_action != "Upd":
                # vals = self.set_default_values(cls, synchro_model, vals)
                if only_minimal:
                    Cache.que_push(backend, "synchro", actual_model, saved_vals, ttl)
            ...
            if synchro_model.auth_action == "Ins":
                rec = -2
            elif vals:
