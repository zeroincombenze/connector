...
class IrModelSynchro(models.Model):
    ...
    @api.model
    def get_ext_id_name(self, backend_id, model, spec=None, force=None):
        """Get local name for external reference
        """
        Cache = self.env["ir.model.synchro.cache"]
        vmodel = self.get_vmodel(model, spec)
        Cache.open(model=vmodel)
        prefix = Cache.get_attr(
            backend_id, "PREFIX") or self.env["synchro.channel"].browse(
            backend_id).prefix
    ...
    def get_loc_ext_id_value(self, backend_id, model, ext_id, spec=None):
        ...
        # Workaround
        if not offset and vmodel == "res.partner.shipping":
            offset = 100000000
        if ext_id < offset:
    ...
    def drop_invalid_fields(self, vmodel, vals, backend_id=None):
        ...
        saved_ext_id = None
        if backend_id:
            ext_id_name = self.get_ext_id_name(backend_id, vmodel)
    ...
    def drop_protected_fields(self, backend, vmodel, vals, rec, no_del_child=False):
        ...
        Cache = self.env["ir.model.synchro.cache"]
        actual_model = self.get_actual_model(vmodel, only_name=True)
        loc_ext_id_name = self.get_ext_id_name(backend_id, vmodel)
    ...
    def get_foreign_text(
        ...
        rec = self.get_rec_by_reference(
            backend.id, actual_model, key_name, value, ctx=ctx, spec=spec
        )
        if rec:
            new_value = rec[0].id
        if not new_value and not no_create and Cache.is_manageable(actual_model):
            new_value = self.create_new_ref(
                backend.id, actual_model, key_name, value, False, ctx=ctx, spec=spec
            )
        self.logmsg(
    ...
    def get_foreign_ref(
        ...
        Cache = self.env["ir.model.synchro.cache"]
        loc_ext_id_name = self.get_ext_id_name(
            backend.id, actual_model, spec=spec, force=True
        )
        new_value = False
        ...
        if is_foreign:
            if spec:
                value_id = self.get_loc_ext_id_value(
                    backend.id, actual_model, value_id, spec=spec
                )
    ...
    def name_from_ref(self, backend_id, vmodel, ext_ref):
        ...
        loc_ext_id_name = self.get_ext_id_name(backend_id, vmodel, force=True)
        counterpart_pk = Cache.get_model_attr(
    ...
    def set_default_values(self, cls, backend, vmodel, vals):
        ...
        Cache = self.env["ir.model.synchro.cache"]
        loc_ext_id_name = self.get_ext_id_name(backend.id, vmodel)
        suppl_key = Cache.get_struct_model_attr(actual_model, "SUPPL_KEY")
    ...
    def create_ext_id(self, backend, actual_model, loc_id, ext_id):
        ext_id_name = self.get_ext_id_name(backend.id, actual_model, force=True)
        ...
        for channel in cache.get_channel_list().copy():
            ...
            for vmodel in model_list:
                ...
                if identity != "odoo" and not cache.get_model_attr(
                    ...
                    continue
                loc_ext_id_name = self.get_ext_id_name(channel_id, vmodel)
                actual_model = self.get_actual_model(vmodel, only_name=True)
    ...
    def pull_full_records(
        ...
        def pull_model(channel_id, select, only_complete, use_workflow, only_minimal,
            ...
            for vmodel in model_list:
                ...
                if len(datas) and isinstance(datas[0], (int, long)):
                    datas.sort()
                ext_id_name = self.get_ext_id_name(channel_id, vmodel)
                for item in datas:
                    if datetime.now() > datetime_stop:
                        break
                    if not item:
    ...
    def pull_record(self, cls, backend_id=None):
        ...
        for rec in cls:
            ...
            for backend in cache.get_channel_list():
                ...
                identity = cache.get_attr(backend_id, "IDENTITY")
                loc_ext_id_name = self.get_ext_id_name(backend_id, model)
                if loc_ext_id_name and hasattr(rec, loc_ext_id_name):
                    vmodel = model
