...
class IrModelSynchro(models.Model):
    ...
    def map_to_internal(
        ...
        for ext_ref in field_list:
            ...
            if (
                (not loc_name or loc_name not in struct)
                and is_foreign and apply4
            ):
                vals = do_apply(
                    backend_id,
                    vals,
            ...
            if loc_name == child_ids:
                ...
                if loc_name in vals or loc_name == "id":
        ...
        prefix = self.env["ir.model.synchro.cache"].get_attr(backend_id, "PREFIX")
    ...
    def assign_backend_loglevel(self, backend):
        Cache = self.env["ir.model.synchro.cache"]
        Cache.set_attr(backend.id, "LOGLEVEL", backend.tracelevel)
        self.LOGLEVEL = {
            "1": "error",
            "2": "info",
            "3": "warning",
            "4": "debug",
        }.get(Cache.get_attr(backend.id, "LOGLEVEL", "2"))
        ...
        if loc_id < 1:
            ...
            if vals:
                ...
                if not do_write and min_vals != vals:
                    ...
                    self.logmsg(
                        "warning", "%s=%s.create(%s)" % (loc_id, actual_model, vals),
                        logrec=self.logrec,
                    )
