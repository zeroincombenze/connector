...
try:
    from python_plus import unicodes, str2bool, _b
...
class IrModelSynchro(models.Model):
    ...
    def create_n_commit(self, model, vals, logrec=None, context=None):
        ...
        try:
            ...
            self.env["synchro.log"].logmsg("trace", "", res_rec=rec, logrec=logrec)
            ...
            self.env["synchro.log"].logmsg(
                "error", "ERROR %s: %s.create(%s)" % (e, model, vals)
            )
        return rec
    ...
    @api.model
    def get_loc_4_ext_key_id(self, backend_id, model, spec=None, force=None):
        ...
        loc_4_ext_key_id = cache.get_model_attr(
            backend_id,
            vmodel,
            "EXT_ID",
        ...
        if not force and not cache.get_struct_model_attr(
            self.get_actual_model(model, only_name=True), loc_4_ext_key_id
        ):
            return ""
        return loc_4_ext_key_id
    ...
    def drop_invalid_fields(self, vmodel, vals, backend_id=None):
        cache = self.env["synchro.cache"]
        saved_ext_id = None
        if backend_id:
            def_ext_key_id = self.get_loc_4_ext_key_id(backend_id, vmodel, force=True)
            ext_key_id = self.get_loc_4_ext_key_id(backend_id, vmodel)
            if def_ext_key_id in vals and def_ext_key_id != ext_key_id:
                saved_ext_id = vals[def_ext_key_id]
        ...
        if saved_ext_id:
            vals[def_ext_key_id] = saved_ext_id
            ...
            protect_update = max(
                ...
                int(
                    cache.get_model_field_attr(
                        backend_id, vmodel, field, "PROTECT", default="0"
                    )
                ),
            )
            if (
                ...
                or (
                    protect_update == 4
                    and rec[field]
                    and int(vals[field]) <= int(rec[field])
                )
                or (protect_update == 2 and rec[field])
                or (protect_update == 1 and not vals[field])
            ):
                del vals[field]
            elif isinstance(vals[field], (basestring, int, float, bool)):
                if (
    ...
    def set_state_to_draft(self, model, rec, vals):
        self.env["synchro.log"].logmsg(
            "debug",
            ">>> %(model)s.set_state_to_draft(%(id)s)",
            res_model=model,
            res_rec=rec,
        )
        errc = 0
    ...
    def create_new_ref(
        ...
        self.env["synchro.log"].logmsg(
            ...
            res_model=actual_model,
            ctx={"key": key_name, "id": value, "ext_id": ext_value},
        )
        ctx = ctx or {}
        cache = self.env["synchro.cache"]
        vmodel = self.get_vmodel(actual_model, spec)
        loc_4_ext_key_id = self.get_loc_4_ext_key_id(backend_id, vmodel)
        suppl_key = cache.get_struct_model_attr(actual_model, "SUPPL_KEY")
        cls = self.env[vmodel]
        vals = {key_name: value}
        if ext_value and loc_4_ext_key_id:
            vals[loc_4_ext_key_id] = self.get_loc_ext_id_value(
    ...
    def do_search(self, actual_model, req_domain, only_id=None, spec=None):
        def atomic_search(cls, domain, has_sequence):
            self.env["synchro.log"].logmsg(
                ...
                res_model=cls.__class__.__name__,
                ctx={"domain": domain},
            )
            if has_sequence:
    ...
    def get_rec_by_reference(
        ...
        loc_4_ext_key_id = self.get_loc_4_ext_key_id(backend_id, vmodel)
        if mode == "tnl":
            translation_model = self.env["synchro.backend.domain.translation"]
            domain = [
                ("model", "=", actual_model),
                ("key", "=", name),
        ...
        if name not in (ext_key_id, loc_4_ext_key_id):
        ...
        if not rec and mode != "tnl" and isinstance(value, str):
    ...
    def get_foreign_ref(
        ...
        cache = self.env["synchro.cache"]
        loc_4_ext_key_id = self.get_loc_4_ext_key_id(
        ...
        if is_foreign:
            ...
            domain = [(loc_4_ext_key_id, "=", value_id)]
    ...
    def get_foreign_value(
        ...
        # TODO: channel_id
        cache.open(
            backend=self.env["synchro.backend"].browse(backend_id), model=relation
        )
    ...
    def name_from_ref(self, backend_id, vmodel, ext_ref):
        ...
        loc_ext_id_name = self.get_loc_4_ext_key_id(backend_id, vmodel, force=True)
        ext_id_name = cache.get_model_attr(backend_id, vmodel, "KEY_ID", default="id")
    ...
    def get_default_n_apply(
        ...
        if not cache.get_attr(backend_id, actual_model):
            # TODO: channel_id
            cache.open(
                backend=self.env["synchro.backend"].browse(backend_id),
                model=actual_model,
            )
        default = cache.get_model_field_attr(
            backend_id, vmodel, loc_name or ".%s" % ext_name, "APPLY", default=""
        )
        if not default:
            default = cache.get_model_field_attr(
                backend_id,
    ...
    def push_ref(
        self, channel_id, cache, vmodel, actual_model, loc_id=None, ext_id=None
    ):
        cache.push_id(channel_id, vmodel, actual_model, loc_id=loc_id, ext_id=ext_id)
        self.env["synchro.log"].logmsg(
            "debug",
    ...
    def map_to_internal(
        self, backend_id, vmodel, vals, no_deep_fields=None, only_minimal=None
    ):
        ...
        def do_apply(
            ...
            ir_apply = self.env["synchro.apply"]
            src = ext_ref
        ...
        def priority_fields(backend_id, vals, loc_ext_id, vmodel):
            cache = self.env["synchro.cache"]
            ...
            for ext_ref in vals.keys():
                ...
                if loc_name != ext_name:
                    only_internal = False
                fields[ext_ref] = (ext_name, loc_name, is_foreign)
        ...
        def cast_type(vals, actual_model, loc_name, ext_ref):
            if (
                ...
                and isinstance(vals[ext_ref], str)
            ...
            ) == "boolean" and isinstance(vals[ext_ref], str):
        ...
        cache = self.env["synchro.cache"]
        actual_model = self.get_actual_model(vmodel, only_name=True)
        loc_ext_id_name = self.get_loc_4_ext_key_id(backend_id, vmodel)
        def_loc_ext_id_name = self.get_loc_4_ext_key_id(backend_id, vmodel, force=True)
        ext_id_name = cache.get_model_attr(backend_id, vmodel, "KEY_ID", default="id")
        child_ids = cache.get_struct_model_attr(
            actual_model, "CHILD_IDS", default=False
        )
        ...
        field_list, only_internal, fields = priority_fields(
            backend_id, vals, def_loc_ext_id_name, vmodel
        )
        ...
        for ext_ref in field_list:
            ...
            #         del vals[loc_name]
            #     continue
            elif isinstance(vals[ext_ref], str) and not vals[ext_ref].strip():
    ...
    def bind_record(
        self, backend_id, vmodel, vals, constraints, chk_in_queue=None, ctx=None
    ):
            ...
            domain = [
                (
                    ...
                    self.get_loc_ext_id_value(
                        backend_id, vmodel, vals[def_loc_ext_id_name]
                    ),
                )
            ]
            rec, maybe_dif = self.do_search(actual_model, domain, only_id=True)
                ...
                for key in keys:
                    if key not in vals:
                        if key == "dim_name" and vals.get("name"):
                            ...
                            domain = []
                            break
                    elif isinstance(vals[key], str) and vals[key] == "":
                        domain.append("|")
            ...
            if rec:
                for field in cache.get_struct_attr(actual_model):
                    ...
                    if field in ("id", "state") or (
                        ...
                        if isinstance(rec[ext_field], (bool, int)):
                        ...
                        elif isinstance(rec[ext_field], str):
                            vals[ext_field] = (
    ...
    def create_ext_id(self, backend_id, actual_model, loc_id, ext_id):
        ext_id_name = self.get_loc_4_ext_key_id(backend_id, actual_model, force=True)
    ...
    def synchro_childs(
        ...
        cache = self.env["synchro.cache"]
        channel = self.env["synchro.backend"].browse(backend_id)
        child_ids = cache.get_struct_model_attr(
            actual_model, "CHILD_IDS", default=False
        )
        model_child = cache.get_struct_model_attr(actual_model, "MODEL_CHILD")
        if not child_ids and not model_child:
            ...
            return -5
        if not self.env["synchro.model"].search(
            [("backend_id", "=", backend_id), ("name", "=", model_child)]
        ):
            if cache.get_attr(backend_id, "IDENTITY") == "odoo":
                self.env["synchro.model"].build_dir_mapper(backend_id, model_child)
            else:
    ...
    def synchro(
        ...
        def browse_from_id(Binder, vals):
            # vmodel?
            id = 0
            rec = None
            if "id" in vals:
                id = vals.pop("id")
            elif hasattr(Binder, "get_id_from_ref"):
                id, xref = Binder.get_id_from_ref(vals)
            if id:
                try:
                    rec = Binder.with_context({"lang": self.env.user.lang}).browse(id)
                except BaseException:
                    pass
        ...
        self.logrec = self.env["synchro.log"].logmsg(
            "trace",
            "synchro(%(vals)s,%(x)s)",
            res_model=vmodel,
            ctx={"vals": vals, "x": chk_in_queue},
        ...
        actual_model = self.get_actual_model(vmodel, only_name=True)
        Binder = self.get_actual_model(vmodel)
        cache = self.env["synchro.cache"]
        channel = self.env["synchro.backend"].assign_backend(vals)
        ...
        if not backend_id:
            ...
            return -6
        self.env["synchro.log"].logmsg(
            "debug", "### assigned channel is %s" % backend_id
        )

        if hasattr(Binder, "CONTRAINTS"):
            constraints = Binder.CONTRAINTS
        else:
            constraints = []
        ...
        # TODO: channel_id
        cache.open(
            backend=self.env["synchro.backend"].browse(backend_id), ext_model=vmodel
        )
        spec = ""
        if vmodel == actual_model:
        ...
        vals, ref_in_queue, parent_child_mode = self.map_to_internal(
            backend_id, vmodel, vals, no_deep_fields=no_deep_fields
        )
        def_ext_id_name = self.get_loc_4_ext_key_id(backend_id, vmodel, force=True)
        ext_id = vals.get(def_ext_id_name)
        ext_id_name = self.get_loc_4_ext_key_id(backend_id, vmodel)
        # if ref_in_queue or vmodel == "ir.module.module":
        if ref_in_queue:
        ...
        loc_id, rec = browse_from_id(Binder, vals)
        if loc_id < 0:
            return loc_id
        elif loc_id == 0:
            loc_id, rec = self.bind_record(backend_id, vmodel, vals, constraints)
        if loc_id == -7 and not has_state:
            pop_ref(backend_id, vmodel, actual_model, False, ext_id)
            self.env["synchro.log"].logmsg(
                "info", "### No values passed(%s.%s)" % (vmodel, actual_model)
            )
            return loc_id
        if (
            ...
            if not self.diff_parent(vals, actual_model):
                self.reset_refs(backend_id, vmodel, actual_model)
                return 0
        if has_state:
            vals, erc = self.set_state_to_draft(vmodel, rec, vals)
            if erc < 0:
        ...
        if loc_id < 1:
            if has_state or has_2delete or not ext_id or parent_child_mode == "C":
                ...
                for nm in self.DEF_INCL_FLDS:
                    if vals.get(nm):
                        min_vals[nm] = vals[nm]
                min_vals = self.set_default_values(cls, backend_id, vmodel, min_vals)
                if not min_vals or min_vals == vals:
                    ...
                    do_write = True
            if vals:
                if hasattr(Binder, "assure_values"):
                    vals = Binder.assure_values(vals, rec)
        ...
        if loc_id > 0 and do_write:
            ...
            try:
                rec = Binder.with_context({"lang": self.env.user.lang}).browse(loc_id)
            except BaseException as e:  # pragma: no cover
                self.env.cr.rollback()  # pylint: disable=invalid-commit
                self.env["synchro.log"].logmsg(
                    "error",
            ...
            if rec:
                ...
                if vals:
                    try:
                        ...
                        self.env["synchro.log"].logmsg(
                            "trace", "", logrec=self.logrec, res_rec=rec
                        )
                    except BaseException as e:  # pragma: no cover
                        self.env.cr.rollback()  # pylint: disable=invalid-commit
                        self.env["synchro.log"].logmsg(
    ...
    def commit(self, cls, loc_id, ext_id=None):
        ...
        if (not loc_id or loc_id < 1) and ext_id:
            loc_id = self.bind_record(1, vmodel, {"id": ext_id}, [], False)
        try:
        ...
        loc_id = 0
        if has_state:
            loc_id = self.set_actual_state(vmodel, rec_2_commit)
            if loc_id < 0:
    ...
    def pull_recs_2_complete(self, only_model=None):
        ...
        def get_ext_id(ext_ix, datas):
            ...
            ext_ix += 1
            if ext_ix < len(datas):
                if isinstance(datas[ext_ix], int):
        ...
        for channel in cache.get_channel_list().copy():
            ...
            identity = cache.get_attr(channel_id, "IDENTITY")
            if identity == "odoo":
                model_list = self.env["synchro.cache"].TABLE_DEF.keys()
            else:
                domain = [("backend_id", "=", channel_id)]
                model_list = [
                    x.name
                    for x in self.env["synchro.model"].search(domain, order="sequence")
                ]
            ctr = 0
            for vmodel in model_list:
                ...
                if identity != "odoo" and not cache.get_model_attr(
                    ...
                    continue
                loc_ext_id_name = self.get_loc_4_ext_key_id(channel_id, vmodel)
                actual_model = self.get_actual_model(vmodel, only_name=True)
                ...
                if not isinstance(datas, (list, tuple)):
                    datas = [datas]
                if len(datas) and isinstance(datas[0], int):
                    datas.sort()
                if loc_ext_id_name:
                    recs = cls.search(
                ...
                while ext_id > 0 and loc_id > 0:
                    if (
                        ...
                        if identity == "odoo" or cache.get_model_attr(
                            channel_id, vmodel, "2PULL", default=False
                        ):
                            if isinstance(datas[ext_ix], int):
                                vals = self.get_model_of_channel(
                                    channel_id, vmodel
    ...
    def pull_full_records(
        ...
        def update_rec_counter(
            ...
            if ext_id > rec_counter:
                ...
                if use_workflow:
                    ...
                    vals = {"rec_counter": rec_counter}
                    rec = self.env["synchro.model"].search(
                        [
                            ("name", "=", model),
                            ("model_spec", "=", False),
                            ("backend_id", "=", cur_backend.id),
                        ]
                    )
                    if rec:
                        rec.write(vals)
        ...
        def do_workflow(
            ...
            cur_channel = self.env["synchro.backend"].browse(channel_id)
        ...
        def pull_model(
            ...
            for vmodel in model_list:
                if datetime.now() > datetime_stop:
                    break
                if not self.env["synchro.model"].search(
                    [("backend_id", "=", channel_id), ("name", "=", vmodel)]
                ):
                    if identity == "odoo":
                ...
                # channel_id, vmodel, "", default="id")
                ext_id_name = self.get_loc_4_ext_key_id(channel_id, vmodel)
                for item in datas:
                    if datetime.now() > datetime_stop:
                        break
                    if not item:
    ...
    def pull_record(self, cls, backend_id=None):
        ...
        for rec in cls:
            ...
            for backend in cache.get_channel_list():
                ...
                identity = cache.get_attr(backend_id, "IDENTITY")
                loc_ext_id_name = self.get_loc_4_ext_key_id(backend_id, model)
                if loc_ext_id_name and hasattr(rec, loc_ext_id_name):
                    vmodel = model
                    ...
                    if identity == "vg7":
                        if model == "res.partner":
                            if ext_id > 200000000:
                                ...
                                cache.open(model=vmodel, backend=channel)
                            ext_id = self.get_actual_ext_id_value(
                                backend_id, vmodel, ext_id
                            )
                    if ext_id and (
                        identity != "vg7" or vmodel != "res.partner.invoice"
                    ):
                        self.pull_1_record(backend_id, vmodel, ext_id)
    ...
    def manage_module(self, vals):
        ...
        if module.state == "uninstalled":
            try:
                ...
                self.env.cr.rollback()  # pylint: disable=invalid-commit
                self.logmsg(
                    "error", "Module %s not installable\n%s" % (vals["name"], e)
                )
                return -4
            max_ctr = len(module.dependencies_id) + 3
            query = (
                "SELECT id FROM ir_module_module WHERE"
