    ...
    def apply_line_vals_from_prod(
        ...
        if vals.get("product_id"):
            ...
            if not vals.get("product_uom"):
                vals = self.apply_uom(
                    backend_id, vals, "product_uom", None, None, vmodel, product=product
                )
            if vmodel == "purchase.order.line" and not vals.get("taxes_id"):
                vals = self.apply_tax(
                    backend_id, vals, "taxes_id", None, None, vmodel, product=product
                )
            elif vmodel == "sale.order.line" and not vals.get("tax_id"):
                vals = self.apply_tax(
                    backend_id, vals, "tax_id", None, None, vmodel, product=product
                )
                ...
                vals = self.apply_tax(
                    backend_id, vals, "tax_ids", None, None, vmodel, product=product
                )
            if vmodel == "account.invoice.line" and not vals.get("account_id"):
                vals = self.apply_account(
                    backend_id, vals, "account_id", None, None, vmodel, product=product
                )
        return vals
